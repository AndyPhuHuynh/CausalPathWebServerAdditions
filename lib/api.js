    ...
    return executeCommandLineProcess(cmd2, rejectEmpty).then(
        (data) => {
            ...
            return runCausalPathForAll(dirs, 0).then(() => {
                ...
                let cmd4 = "find ./analysisOut/" + room + ' -name "invalid*"'; //now find the analysis files
                return executeCommandLineProcess(cmd4, rejectEmpty).then(
                    (data2) => {
                        console.log("Analysis was unsuccessful");
                        console.log(cmd4);
                        console.log("Error parsing parameters or data files");
                        return data2;
                    }
                ).catch(
                        executeCommandLineProcess(cmd3, rejectEmpty).then(
                        (data3) => {
                            console.log("Analysis is successful.");
                            console.log(cmd3);
                            console.log("In analysis zip found causative.json as " + data3);
                            return data3;
                        },
                        (error) => {
